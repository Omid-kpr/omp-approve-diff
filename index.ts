...
export default function showDiffsExtension(pi: ExtensionAPI) {
  ...
  function updateStatus(ctx: ExtensionContext) {
    ...
    ctx.ui.setStatus(
      ...
      config.autoApprove
        ? ctx.ui.theme.fg(
            "warning",
            t("status.auto", "✍ auto-approve file changes"),
          )
        : undefined,
    );
  }
  ...
  function setConfig(
    next: Partial<DiffApprovalConfig>,
  ...
  function setAutoApprove(autoApprove: boolean, ctx: ExtensionContext) {
    setConfig(
      ...
      true,
      autoApprove
        ? t("notify.autoOn", "Auto-approve is ON for file changes.")
        : t("notify.manualOn", "Manual diff review is ON."),
    );
  }

  function setDiffColorMode(
  ...
  async function handleKeybindingsMenu(ctx: ExtensionContext) {
    ...
    const options = [
      ...actions.map(
        (action) =>
          `${keybindingLabel(action)}: ${formatKeys(config.keybindings[action])}`,
      ),
    ...
    const selectedAction = actions.find((action) =>
      kbChoice.startsWith(keybindingLabel(action)),
    );
    ...
    setConfig(
      { keybindings: { ...config.keybindings, [selectedAction]: newKeys } },
      ctx,
    );
  }
    ...
    const items: SettingItem[] = [
      {
        ...
        values: ["off", "on"],
        description:
          "When on, file changes apply without opening the diff review modal.",
      },
      {
        id: "diffColorMode",
        label: "Diff colors",
        ...
        description:
          "default = omp-approve-diff red/green backgrounds; theme = active pi theme success/error backgrounds.",
      },
      {
        id: "showDiffRail",
        label: "Diff rail",
        ...
        values: ["off", "on"],
        description:
          "Show a colored rail marker beside each rendered diff line.",
      },
      {
        id: "expandableLayout",
        ...
        values: ["off", "on"],
        description:
          "When on, the diff opens inline and Ctrl+F expands it to an overlay.",
      },
      {
        id: "collapsedHeight",
        label: "Collapsed height",
        currentValue: config.collapsedHeight,
        values: valuesWithCurrent(config.collapsedHeight, [
          "20%",
          "30%",
          "40%",
          "50%",
      ...
      },
      {
        ...
        values: valuesWithCurrent(config.expandedHeight, [
          "80%",
          "90%",
          "100%",
        ]),
        description:
          "Maximum overlay height after pressing Ctrl+F in expandable layout.",
      },
      {
        id: "expandedWidth",
        label: "Expanded width",
        currentValue: config.expandedWidth,
        values: valuesWithCurrent(config.expandedWidth, [
          "80%",
          "90%",
          "96%",
          "100%",
        ...
        description:
          "Overlay width after pressing Ctrl+F in expandable layout.",
      },
      {
        id: "keybindings",
        label: "Keybindings",
    ...
    await ctx.ui.custom((tui, theme, _kb, done) => {
      const settingsList = new SettingsList(
        ...
        (id, newValue) => {
          ...
          if (id === "diffColorMode") {
            setConfig(
              { diffColorMode: newValue === "theme" ? "theme" : "default" },
              ctx,
              false,
            );
          }
      ...
      return {
        render: (width: number) => [
          truncateToWidth(
            theme.fg("accent", theme.bold("omp-approve-diff settings")),
            width,
            "",
            false,
          ),
          truncateToWidth(
            theme.fg("muted", `Config: ${CONFIG_PATH}`),
            width,
            theme.fg("muted", "…"),
            false,
          ),
  ...
  async function handleCommand(args: string, ctx: ExtensionContext) {
    ...
    if (
      ...
      ["color", "colors", "diff-colors", "diff-color-mode"].includes(
        commandParts[0]!,
      ) &&
  ...
  function queuePendingImmediateApply(
    toolCallId: string,
    preview: ChangePreview,
    afterText: string,
  ) {
  ...
  async function restoreReviewedFinalContent(
    absolutePath: string,
    afterText: string,
  ) {
    try {
  ...
  function buildReviewedEditMessage(
    preview: ChangePreview,
    afterText: string,
  ): string {
    if (wasUserEdited(preview, afterText)) {
      return `Successfully applied user-reviewed and user-edited final contents to ${preview.path} (read the current state of file to underestand what user did edit on top of your proposed edit).`;
    }
    return `Successfully applied reviewed final contents to ${preview.path}.`;
  }

  function buildReviewedWriteMessage(
    preview: ChangePreview,
    finalContent: string,
    afterText: string,
  ): string {
    if (wasUserEdited(preview, afterText)) {
      return `Successfully wrote ${Buffer.byteLength(finalContent, "utf-8")} bytes of user-reviewed and user-edited final contents to ${preview.path} (read the current state of file to underestand what user did edit on top of your proposed write).`;
    }
    return `Successfully wrote ${Buffer.byteLength(finalContent, "utf-8")} bytes to ${preview.path}`;
  }

  function buildReviewedResultDetails(
    preview: ChangePreview,
    afterText: string,
  ) {
  ...
  async function applyReviewedAfterText(
    preview: ChangePreview,
    afterText: string,
  ): Promise<any> {
    const finalContent = await restoreReviewedFinalContent(
    ...
    if (preview.toolName === "edit") {
      return {
        content: [
          { type: "text", text: buildReviewedEditMessage(preview, afterText) },
        ],
  ...
  pi.on("tool_call", async (event, ctx) => {
    if (!ctx.hasUI) return;
    if (!TOOL_CALL_REVIEWED_TOOLS.has(event.toolName as PreviewToolName))
      return;
    ...
    if (decision.action === "reject" || decision.action === "steer") {
      const feedback =
        decision.action === "steer" ? decision.feedback?.trim() : undefined;
    ...
    if (decision.afterTextOverride !== undefined) {
      ...
      queuePendingImmediateApply(
        event.toolCallId,
        preview,
        decision.afterTextOverride,
      );
    }
  });
